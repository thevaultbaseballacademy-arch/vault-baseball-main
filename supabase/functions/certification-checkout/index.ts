...
serve(async (req) => {
  ...
  try {
    ...
    const { priceId, certificationLabel } = await req.json();
    if (!priceId) throw new Error("Price ID is required");
    logStep("Request parsed", { priceId, certificationLabel });
    ...
    return new Response(JSON.stringify({ error: "An error occurred during checkout. Please try again." }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 500,
    });
  }
